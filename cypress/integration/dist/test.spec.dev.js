...
describe("Does the display numbers when the buttons are clicked", function () {
...
describe("Can it perform calculations", function () {
  ...
  it("Does it display a running total of 49 when buttons '7' 'x' '7' '+' are clicked", function () {
    // Arrange
    cy.visit("http://127.0.0.1:5501/index.html"); // Act
    // click 9

    cy.get("[data-cy=7]").click(); // click x

    cy.get("[data-cy=x]").click(); // click 9

    cy.get("[data-cy=7]").click(); // click =

    cy.get("[data-cy='+']").click(); // Assert

    cy.get("[data-cy=display]").should("have.text", "49");
  });
  it("Does it perform a complex calculation when buttons '7' 'x' '7' '+' '100'are clicked", function () {
    // Arrange
    cy.visit("http://127.0.0.1:5501/index.html"); // Act
    // click 9
    ...
    cy.get("[data-cy='1']").click(); // click +

    cy.get("[data-cy='0']").click(); // click +
...
describe("Does it detect nummeric keypad presses", function () {
  it("Does it display 789 when the numeric keypad keys 789 are pressed", function () {
    // Arrange
    cy.visit("http://127.0.0.1:5501/index.html"); // Act
    // Type 789 on the numeric keypad

    cy.get("body").type('789'); // Assert

    cy.get("[data-cy=display]").should("have.text", "789");
  });
  it("Does it display 78 when the numeric keypad keys 7488/96= are pressed", function () {
    // Arrange
    cy.visit("http://127.0.0.1:5501/index.html"); // Act
    // Type 789/96= on the numeric keypad

    cy.get("body").type('7488/96='); // Assert
